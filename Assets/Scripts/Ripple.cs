...
public class Ripple : MonoBehaviour
{
    ...
    {
        while (isTargetInView && ripplesVisible)
        {
            UpdateTargetDetection();
            ...
            {
                if (!ripplesVisible) break; // Exit if visibility is turned off

                TriggerSingleCircle();
                yield return new WaitForSeconds(emitCooldown);

                // If the target gets out of view or ripples become invisible, stop
                //UpdateTargetDetection();
                if (!isTargetInView || !ripplesVisible)
                {
                    batchCoroutine = null;
                    yield break;
                }
    ...
    void HandleViewStateChanges()
    {
        if (lastInViewState != isTargetInView)
        {
            ...
            if (isTargetInView && autoTriggerWhenOutOfView && ripplesVisible)  // ✅ only when in view and visible
            {
                TriggerSingleCircle();
            }
        }
    ...
    {
        if (isTargetInView && Time.time - lastEmitTime > emitCooldown && ripplesVisible)
        {
            TriggerSingleCircle();
    ...
    private IEnumerator MoveSingleCircle1()
    {
        ...
        if (numParticles > 0)
        {
            ...
            while (elapsedTime < journeyTime && elapsedTime < circleLifetime && ripplesVisible)
            {
